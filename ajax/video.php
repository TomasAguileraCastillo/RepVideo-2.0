        ...
        $data = Array();
            ...
            while($reg = $rspta->fetch_object()){
                $data[]=array(
                    ...
                    "0"=>($reg->condicion_videos) ? '<button class="btn btn-primary " onclick="mostrar('.$reg->id_videos.')"> <i class="fa fa-pencil"></i></button>'.
                    ' <button class="btn btn-danger " onclick="desactivar('.$reg->id_videos.')"> <i class="fa fa-close"></i> </button>':
                    '<button class="btn btn-primary" onclick="mostrar('.$reg->id_videos.')"> <i class="fa fa-pencil"></i> </button>'.
                    ' <button class="btn btn-success" onclick="activar('.$reg->id_videos.')"> <i class="fa fa-check"></i> </button>',
                    "1"=>$reg->nombre_videos,
                    "2"=>$reg->descripcion_videos,
                    "3"=>$reg->comentario_videos,
                    "4"=>$reg->fechasubida_videos,
                    "5"=>$reg->subidopor_videos,
                    "6"=>$reg->curso_videos,
                    "7"=>//"<a href='../vistas/Reproductor/reproductor.php?saludo=".$reg->ubicacion_video." ' target='_blank'>link</a>
                         "<div class='input-group'>
                            <a class='btn btn-outline-secondary border-0' href='../vistas/Reproductor/reproductor.php?saludo=".$reg->ubicacion_videos." ' target='_blank' role='button'> <i class='bi bi-lightning-charge-fill'></i></a> 
                            <input type='hidden'   name='target1' id='target1' value='../vistas/Reproductor/reproductor.php?saludo=".$reg->ubicacion_videos." '>
                            <button type='button' class='btn btn-outline-secondary border-0' onclick='copiar()'><i class='bi bi-clipboard'></i></button>
                        </div>",
...
/*link Admin*/      "8"=>//"<a class='btn btn-outline-secondary border-0' href='../files/".$reg->ubicacion_video."' target='_blank' rel='noopener noreferrer' >link</a>",
                    "<div class='input-group'>
                    <a class='btn btn-outline-secondary border-0' href='../files/".$reg->ubicacion_videos."'  target='_blank' role='button'>
                    <i class='bi bi-lightning-charge-fill'></i></a></div>",
                    "9"=>"  <video width='100%' height='35'>
                                <source src='../files/".$reg->ubicacion_videos."'>
                            </video>",
                    "10"=>($reg->condicion_videos)?
